...
//            spamArrows(livingEntity);
    ...
    private void handleLookDirection(double xRotOffset, double yRotOffset, double pitchFacingTarget, double yawFacingTarget) {
        this.mob.setXRot((float) Mth.clamp(Math.toDegrees(pitchFacingTarget) + 90 * yRotOffset, -90, 90));
        this.mob.setYRot((float) Math.toDegrees(normalizeAngle(yawFacingTarget + Math.PI * xRotOffset)));
//        this.mob.setXRot((float) (180 * x));
//        this.mob.setYRot((float) (90 * y));
    }
